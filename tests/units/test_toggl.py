...
class TestEvaluator(TestCase):
    ...
    def test_averages_with_active_entry(self):
        mocked1 = get_mocked_entry(duration=10, week=1, day=1)
        mocked2 = get_mocked_entry(duration=10, week=1, day=2)
        mocked3 = get_mocked_entry(duration=10, week=2, day=2)
        mocked4 = get_mocked_entry(duration=-10, week=2, day=2)
        week1_average = 20 / 5
        week2_average = 10 / 5
        month1_average = (week1_average + week2_average) / 4
        mocked_entries = [mocked1, mocked2, mocked3, mocked4]
    ...
    def test_get_until_week_average(self):
        ...
        mocked_entries = [mocked1, mocked2, mocked3]
        mocked_evaluator = evaluator.Evaluator(mocked_entries)
        result = mocked_evaluator._get_week_average_until(mocked2.start)
    ...
    def test_get_until_month_average(self):
        ...
        mocked_entries = [mocked1, mocked2, mocked3]
        mocked_evaluator = evaluator.Evaluator(mocked_entries)
        result = mocked_evaluator._get_month_average_until(mocked2.start)
